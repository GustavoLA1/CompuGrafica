...
const int MAX_OBJECT_AREA = FRAME_HEIGHT*FRAME_WIDTH / 1.5;//FRAME_HEIGHT*FRAME_WIDTH / 1.5;
...
int main(void)
{
	...
	while ((key = waitKey(30)) != 27)
	{
		...
		//cout<<"x: "<<x<<" y: "<<y<<endl;
		inRange(hsvframe, Scalar(H_MIN,S_MIN,V_MIN)/*min*/, Scalar(H_MAX,S_MAX,V_MAX)/*max*/, rangeframe); //colores de la imagen a capturar(blanco)
		
		
		circle(frame, Point(x, y), 15, Scalar(255, 0, 0), -1);
		putText(frame, "50", Point(x - 20, y + 15), FONT_HERSHEY_PLAIN, 3, CV_RGB(10, 255, 0), 5);
		//putText(frame, dedos, Point(450, 180), FONT_HERSHEY_SIMPLEX, 3, CV_RGB(100, 255, 0), 5);
		circle(frame, Point(420, 250), 30, Scalar(255, 0, 0), -1);
		putText(frame, "+", Point(420 - 20, 250 + 15), FONT_HERSHEY_PLAIN, 3, CV_RGB(10, 255, 0), 5);
		circle(frame, Point(500, 250), 30, Scalar(255, 0, 0), -1);
		putText(frame, "-", Point(500 - 20, 250 + 15), FONT_HERSHEY_PLAIN, 3, CV_RGB(10, 255, 0), 5);
		rectangle(frame, Point(650, 120), Point(1200, 400), Scalar(255, 255, 0), 1, 10, 1);
		cout << val << endl;
		if (x <= 500 && x >= 350 && y <= 500 && y >= 350){
			//val2='0'+val;
			if (val <= 0)val = 1 + rand() % (100);
			rectangle(frame, Point(800, 800), Point(500, 500), Scalar(255, 255, 0), 1, 10, 1);
			putText(frame, to_string(val), Point(400, 180), FONT_HERSHEY_SIMPLEX, 5, CV_RGB(100, 255, 0), 5);
...
void morphit(Mat &img)
{
	erode(img, img, getStructuringElement(MORPH_RECT, Size(9, 9)));
	dilate(img, img, getStructuringElement(MORPH_RECT, Size(22, 22)));
}
void blurthresh(Mat &img)
{
...
void trackFilteredObject(int &x, int &y, Mat threshold, Mat &cameraFeed)
{
	...
	if (hierarchy.size() > 0) {
		...
		if (numObjects < MAX_NUM_OBJECTS){
			...
			if (objectFound == true){
				putText(cameraFeed, "Buscando Objeto", Point(0, 50), 2, 1, Scalar(0, 255, 0), 2);
		...
		else putText(cameraFeed, "", Point(0, 50), 1, 2, Scalar(0, 0, 255), 2);
	}
}
///////////////////////------------------------------------------
...
void workOnDefects(Mat &frame, vector<Point>&fingertips, vector<Point>&deptharr)
{
	...
	if (fingertips.size()<=5)
	{
		...
		{
			dedos=to_string(fingertips.size());
	...
	//	SendInput(1, &ip, sizeof(INPUT));*/
	//	putText(frame, position, Point(400, 50), 1, 2, CV_RGB(255, 0, 0), 2, 8);
	//	SetCursorPos(1366/400*(middle.x-100),800/200*(middle.y-200));//cuando se mueve
	//}
